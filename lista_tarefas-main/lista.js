...
frm.btSelecionar.addEventListener("click", () => {  // seleciona o valor inserido no campo
  const tarefas = document.querySelectorAll("h5")  

  if (tarefas.length == 0) {                     //verifica se foi adicionado valor no campo
    alert("Não há tarefas para selecionar")     // retorna um alerta na tela se nao inserir valor
    return                                        
  }

  let aux = -1   // atribui valor para variavel

 
  for (let i = 0; i < tarefas.length; i++) {  // cria um for para armazenar as tarefas digitadas
    
    if (tarefas[i].className == "tarefa-selecionada") { // 
      tarefas[i].className = "tarefa-normal"  // Define nome como uma tarefa normal    
      aux = i                                     
      break                                       
    }
  }
